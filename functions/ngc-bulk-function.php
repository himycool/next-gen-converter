...
function convert_images()
{
   ...
   // print_r($_POST);
    ...
    $perRun = $_POST['perRun'];
    // //$rmvImg = $_POST['rmvImg'];
    $bulkFrom = $_POST['bulkFrom'];
    // $form_data= $_POST['form_data']; //var_dump($form_data);
    ...
    if($bulkFrom == "upload"){
        writeLog(' upload images start', 'success');
        $dir = plugin_dir_path( __DIR__ )."images/";
        $imgCount = count($_FILES);

        for ($i = 0; $i < $imgCount; $i++) {
           // chmod($dir . $_FILES['bulk-files-'.$i]['name'], 0755);
            move_uploaded_file($_FILES['bulk-files-'.$i]['tmp_name'], $dir . $_FILES['bulk-files-'.$i]['name']); //upload image to server
        ...
        if ($queryAtt->have_posts()) {
            while ($queryAtt->have_posts()) { $queryAtt->the_post();
                ...
                if(file_exists($destination)){
                    ...
                    if($convertImg){
                        $attachment = array(
                            'post_mime_type' => ($outFormat=="webp") ? 'image/webp' : 'image/avif',
